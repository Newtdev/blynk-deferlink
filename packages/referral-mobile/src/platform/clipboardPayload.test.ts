...
test('parses a well-formed, fresh payload (no token)', () => {
  const raw = `deferlink_ref:v1:ABC123:${NOW_SECONDS}`;
...
test('parses a well-formed payload with a token', () => {
  const raw = `deferlink_ref:v1:ABC123:${NOW_SECONDS}:click-id.1755000000.deadbeef`;
  const result = parseClipboardReferralPayload(raw, WINDOW, NOW);
...
test('rejects a payload missing the timestamp segment', () => {
  assert.equal(parseClipboardReferralPayload('deferlink_ref:v1:ABC123', WINDOW, NOW), null);
});

test('rejects a payload with a non-numeric timestamp', () => {
  assert.equal(parseClipboardReferralPayload('deferlink_ref:v1:ABC123:not-a-number', WINDOW, NOW), null);
});

test('rejects a payload older than maxAgeSeconds', () => {
  const stale = NOW_SECONDS - WINDOW - 1;
  assert.equal(parseClipboardReferralPayload(`deferlink_ref:v1:ABC123:${stale}`, WINDOW, NOW), null);
});

test('accepts a payload exactly at the age boundary', () => {
  const atEdge = NOW_SECONDS - WINDOW;
  const result = parseClipboardReferralPayload(`deferlink_ref:v1:ABC123:${atEdge}`, WINDOW, NOW);
...
test('rejects a payload claiming to be from the future (clock skew)', () => {
  ...
  assert.equal(parseClipboardReferralPayload(`deferlink_ref:v1:ABC123:${future}`, WINDOW, NOW), null);
});
...
test('a referral code itself may contain colons without breaking parsing', () => {
  ...
  const raw = `deferlink_ref:v1:AB:C123:${NOW_SECONDS}`;
  const result = parseClipboardReferralPayload(raw, WINDOW, NOW);
...
test('a colon-containing code and a dot-delimited token together still parse correctly', () => {
  ...
  const raw = `deferlink_ref:v1:AB:C123:${NOW_SECONDS}:click-id.1755000000.deadbeef`;
  const result = parseClipboardReferralPayload(raw, WINDOW, NOW);
